...
- ACCEPTED (no negative message): this is a SILENT ACCEPT. Set Status "Silent Accept" (create the label with createLabelsIfMissing if needed), set Channel "Connection Request", LEAVE the lead in its awaiting group (no group move), do NOT assign Owner, and do NOT notify Dan. The first message in Outreach Drafts stays queued for whenever Dan works the Silent Accept list. Do NOT send it.
...
Catch silent accepts every run, and check BOTH awaiting groups: "Outreach Sent (Awaiting)" (group_mm3w1p1z, signal leads) and "Alongside Campaign (Awaiting)" (group_mm422a7v, the cold Alongside campaign), plus the "Cold Leads - No Invite Response" group (group_mm43vd24, aged-out invites that must keep being checked). Treat them all identically for acceptance detection, silent-accept reconciliation, and reply routing. Some people accept the connection and never send a message, which produces no email and no inbox thread, so they sit at "Invite Pending" forever unless you reconcile against your actual connections. With the Claude in Chrome tools, open https://www.linkedin.com/mynetwork/invite-connect/connections/ (My Connections, sorted by Recently added) and capture the people added since the last run (the most recent 40 to 120 covers a daily cadence). Optionally also open https://www.linkedin.com/mynetwork/invitation-manager/sent/ to see which sent invitations are still pending; anyone no longer pending has accepted or expired. For each board lead still at "Invite Pending", check whether that person now appears in your recently-added connections, matching carefully by name and org. For each confirmed match who has NOT messaged (no thread, or a thread that shows only Dan's note with no reply), handle them QUIETLY (per Dan, June 7, 2026): set Status "Silent Accept", set Channel "Connection Request", LEAVE the item in its awaiting group (no group move), do NOT assign Owner, and do NOT notify Dan; Silent Accepts are never surfaced as needing attention and do not belong in the Needs Me Now lens. Record the evidence pointer "appeared in recently-added connections on this date" in LinkedIn Insights. Never send anything. If a matched person also has a real reply in the inbox, route them as MESSAGED ("Replied (Triage)") per Step 3 instead of as a fresh accept.

## Aging: Invite Sent - No Response (Cold Leads)

On every run, AFTER acceptance detection and the silent-accept reconciliation, age out stale invites: any lead with Status "Invite Pending" in EITHER awaiting group (Outreach Sent (Awaiting) group_mm3w1p1z or Alongside Campaign (Awaiting) group_mm422a7v) whose Date Detected (date_mm3vbbqb) is 5 or more days in the past gets Status "Invite Sent - No Response" (pass createLabelsIfMissing true) and moves to the "Cold Leads - No Invite Response" group (group_mm43vd24). No comment, no Owner, no notification; this is routine hygiene. ALSO age stale follow-ups: any lead with Status "Followed Up (Awaiting Reply)" in the Outreach Sent (Awaiting) group (group_mm3w1p1z) whose Date Detected is 5 or more days in the past gets Status "Followed Up - No Response" (pass createLabelsIfMissing true) and moves to the same Cold Leads group. These leads already connected, so monitor them for REPLIES only, not invite accepts; a reply routes them to "Replied (Triage)" like any other cold lead. COLD LEADS STAY MONITORED: include the Cold Leads group in every acceptance check, silent-accept reconciliation, reply scan, and SEND: pass. If a cold lead ACCEPTS with no message, set Status "Silent Accept" and move it BACK to its original awaiting group (Outreach Sent (Awaiting) for signal leads, Alongside Campaign (Awaiting) for campaign leads); if a cold lead REPLIES, route it to "Replied (Triage)" and group_mm3w6sdc exactly like any other reply. An acceptance always puts the lead back in the normal flow.
